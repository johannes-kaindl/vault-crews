...
**Files:** Create: komplettes Scaffold aus `../../_docs/templates/obsidian-plugin/` (Workspace-Repo `_docs`, vom Repo-Root aus) (esbuild.config.mjs, eslint.config.mjs, tsconfig.json, tsconfig.test.json, vitest.config.ts, package.json, manifest.json, versions.json, scripts/, tests/) → angepasst auf `vault-crews`.
...
**Files:** Create: `src/vendor/kit/{sse,think,endpoint,i18n}.ts`, `tests/__mocks__/obsidian.ts` — Quellen: Schwester-Repo `obsidian-kit` (`../obsidian-kit/src/pure/*`) und `src/testing/obsidian-mock.ts` (reale Dateinamen dort prüfen und mappen).
...
**Referenz:** Schwester-Repo `vault-rag` (`../vault-rag/src/`) (SSE-über-Transport-Muster, `/api/v0/models`-Feldnamen live prüfen).
