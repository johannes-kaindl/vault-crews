...
function summaryFromResult(result: RunResult, writes: string[], abortRequested: boolean): SummaryVM {
  return {
    ...
    primaryLabel: result.status === "ok" || (result.writes === 0 && result.emptyCollector != null) ? t("panel.openLog") : t("panel.viewFailure"),
...
function summaryFromLastRun(s: RunSummary): SummaryVM {
  return {
    ...
    abortNote: null,
    primaryLabel: s.status === "ok" || (s.writes === 0 && s.emptyCollector != null) ? t("panel.openLog") : t("panel.viewFailure"),
    undoable: s.undoable,
    undoLabel: t("panel.undo"),
    nextActionLabel: t("panel.nextAction"),
